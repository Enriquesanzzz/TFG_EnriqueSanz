...
# Directorio donde están los JSONs
...
with open(output_csv, "w", newline="", encoding="utf-8") as csvfile:
    ...
    for filename in os.listdir(json_folder):
        if filename.endswith(".json"):
            ...
            video = data["original_file"]
            status = data["status"]
            facial = get_value(data, "data", "facial", "average_emotions")
            traits = get_value(data, "data", "traits")
            voice = get_value(data, "data", "voice", "frequencies")
            if not isinstance(voice, dict):
                voice = {}
            voice_emotions = get_value(data, "data", "voice", "emotions")
            if not isinstance(voice_emotions, dict):
                voice_emotions = {}
            speech = get_value(data, "data", "speech")
            tense = get_value(speech, "tense")
            sentiment = get_value(speech, "sentiment")
            ...
            row = [
                ...
                get_value(data, "data", "facial", "most_frequent_dominant_emotion"),
                get_value(data, "data", "facial", "dominant_emotion_counts", "surprise"),
                get_value(data, "data", "facial", "average_face_confidence"),
                ...
                traits.get("survival", "null"),
                traits.get("creativity", "null"),
                traits.get("self_esteem", "null"),
                traits.get("compassion", "null"),
                traits.get("communication", "null"),
                traits.get("imagination", "null"),
                traits.get("awareness", "null"),
                ...
                get_value(tense, "past"),
                get_value(tense, "present"),
                get_value(tense, "future"),
                get_value(sentiment, "polarity"),
                get_value(sentiment, "subjectivity"),
